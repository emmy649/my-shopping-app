...
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    const uncompletedTodayTasks = tasks.filter(
      (t) => isSameDay(new Date(t.date), today) && !t.done
    );
    ...
    if (Notification.permission === 'granted') {
      if (uncompletedTodayTasks.length > 0) {
        const lines = uncompletedTodayTasks.map((t) => `• ${t.text}`).join('\n');
        new Notification('Задачи за днес:', {
          body: lines,
        });
      }
  
      if (uncompletedTomorrowTasks.length > 0) {
